...
class FocalLoss(nn.Module):
    ...
    def forward(self, inputs, targets):
        #print("input",inputs.shape,targets.shape)

        bce_loss = F.cross_entropy(inputs.squeeze(),  targets,reduction='none')
        loss = self.alpha * (1 - torch.exp(-bce_loss)) ** self.gamma * bce_loss
        return loss
#from https://github.com/hongxin001/logitnorm_ood/blob/0a60eeffb7dfc970fe68e07c5649ea1c9c8244c6/common/loss_function.py
class LogitNormLoss(nn.Module):
    ...
    def forward(self, x, target):
        ...
        return F.cross_entropy(logit_norm, target,reduction='none')

def mse_logitNormLoss(out, targets):
    t = 1
    ...
    if out.numel()==0:
        ...
        logit_norm = torch.div(out, norms) / t
        return torch.squeeze(F.mse_loss(logit_norm,targets,reduction='none'))
        #return F.mse_loss(torch.t(logit_norm),torch.t(targets),reduction='none')
        #losses = (logit_norm - targets)**2
