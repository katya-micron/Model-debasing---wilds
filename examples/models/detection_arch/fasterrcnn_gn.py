...
from examples.models.resnet_GN_ws import ResNet50
#from examples.models.resnet_GN_SiLu import ResNet50
model_urls = {
...
def fastrcnn_loss(class_logits, box_regression, labels, regression_targets):
    ...
    for class_logits_, box_regression_, labels_, regression_targets_ in zip(class_logits, box_regression, labels, regression_targets):
        #KG: logitNorm
        #print("loss",class_logits_.shape, labels_.shape)
        #norms = torch.norm(class_logits_, p=2, dim=-1, keepdim=True) + 1e-7
        #logit_norm = torch.div(class_logits_, norms) / 0.01
        #classification_loss.append(F.cross_entropy(logit_norm, labels_))
...
class GroupNorm32(torch.nn.Module):
    def __init__(self,num_features: int,):
        super().__init__()
        #_log_api_usage_once(self)
        self.gn = nn.GroupNorm(num_groups=32, num_channels=num_features)
    def forward(self,x):
        return  self.gn(x)
def resnetGN_fpn_backbone(backbone_name,weights,norm_layer= nn.BatchNorm2d,
...
def _resnet_fpn_extractor(
    ...
    for name, parameter in backbone.named_parameters():
        if all([not name.startswith(layer) for layer in layers_to_train]) and not name.startswith("fc"):
            print("frozen layes -->",name,trainable_layers)
...
def fasterrcnn_resnet50_fpn(pretrained=False, progress=True,
    ...
    backbone = resnetGN_fpn_backbone('resnet50', pretrained_backbone,norm_layer=GroupNorm32, trainable_layers=trainable_backbone_layers)
    ...
    if pretrained:
        ...
        #                                      progress=progress)
        '''
        state_dict = torch.load('/home/katyag/wilds/faster_rcnn_r50_fpn_gn_ws-all_1x_coco_20200130-613d9fe2.pth') 
        state_dict['state_dict'] = dict({k.replace("backbone","backbone.body"):v for k,v in state_dict['state_dict'].items() }) 
        state_dict['state_dict'] = dict({k.replace("gn","bn"):v for k,v in state_dict['state_dict'].items() }) 
        
        #state_dict['state_dict'] = dict({k.replace("gn1","bn1.gn"):v for k,v in state_dict['state_dict'].items() }) 
        #state_dict['state_dict'] = dict({k.replace("gn2","bn2.gn"):v for k,v in state_dict['state_dict'].items() }) 
        #state_dict['state_dict'] = dict({k.replace("gn3","bn3.gn"):v for k,v in state_dict['state_dict'].items() }) 
        #state_dict['state_dict'] = dict({k.replace("downsample.1","downsample.1.gn"):v for k,v in state_dict['state_dict'].items() }) 
        state_dict['state_dict'] = dict({k.replace("rpn_head.rpn_","rpn.head."):v for k,v in state_dict['state_dict'].items() }) 
        '''
        '''
        state_dict = torch.load('/home/katyag/wilds/resnet50_GNws_8xb32_silu_in1k.pth')  
        state_dict['state_dict'] = dict({k.replace("backbone.","backbone.body."):v for k,v in state_dict['state_dict'].items() }) 
        state_dict['state_dict'] = dict({k.replace("gn1","bn1"):v for k,v in state_dict['state_dict'].items() }) 
        state_dict['state_dict'] = dict({k.replace("gn2","bn2"):v for k,v in state_dict['state_dict'].items() }) 
        state_dict['state_dict'] = dict({k.replace("gn3","bn3"):v for k,v in state_dict['state_dict'].items() }) 
        
        '''
         
        state_dict = torch.load('/home/katyag/wilds/resnet50_gn_ws-15beedd8.pth')
        state_dict['state_dict'] = dict({'backbone.body.'+k:v for k,v in state_dict['state_dict'].items() }) 
        state_dict['state_dict'] = dict({k.replace("gn1","bn1"):v for k,v in state_dict['state_dict'].items() }) 
        state_dict['state_dict'] = dict({k.replace("gn2","bn2"):v for k,v in state_dict['state_dict'].items() }) 
        state_dict['state_dict'] = dict({k.replace("gn3","bn3"):v for k,v in state_dict['state_dict'].items() }) 
        
        '''
        #state_dict = torch.load('/home/katyag/wilds/resnet50_gn-9186a21c.pth')  
        state_dict['state_dict'] = dict({'backbone.body.'+k:v for k,v in state_dict['state_dict'].items() }) 
        state_dict['state_dict'] = dict({k.replace("gn1","bn1.gn"):v for k,v in state_dict['state_dict'].items() }) 
        state_dict['state_dict'] = dict({k.replace("gn2","bn2.gn"):v for k,v in state_dict['state_dict'].items() }) 
        state_dict['state_dict'] = dict({k.replace("gn3","bn3.gn"):v for k,v in state_dict['state_dict'].items() }) 
