...
import examples.models.bit_models as bit_models
from examples.models.bit_models import tf2th
model_urls = {
...
class RoIHeadsWILDS(RoIHeads):
    ...
    def forward(self,
        ...
        #print("cls_loss:",loss_classifier,"box_loss:",loss_box_reg)
...
def resnetGN_fpn_backbone(backbone_name,weights,norm_layer= nn.BatchNorm2d,
    ...
    backbone = bit_models.KNOWN_MODELS[backbone_name]()
...
def _resnet_fpn_extractor(
    ...
    layers_to_train = ["body.block4", "body.block3", "body.block2", "body.block1", "root.conv1"][:trainable_layers]
    if trainable_layers == 5:
        layers_to_train.append("bn1")

    for name, parameter in backbone.named_parameters():
        if all([not name.startswith(layer) for layer in layers_to_train]) and not name.startswith("head"):
    ...
    in_channels_stage2 = 256 #64 // 8
    in_channels_list = [in_channels_stage2 * 2 ** (i - 1) for i in returned_layers]
    out_channels = 256
    return MyBackboneWithFPN(backbone, return_layers, in_channels_list, out_channels, extra_blocks=extra_blocks, zero_head =True)
class MyBackboneWithFPN(nn.Module):
    ...
    def forward(self, x: Tensor) -> Dict[str, Tensor]:
        x_orig = x
        _, layer = self.backbone.get_feats(x)
        x = self.fpn(layer)
        return x
...
def fasterrcnn_resnet50_fpn(pretrained=False, progress=True,
    ...
    backbone = resnetGN_fpn_backbone('BiT-M-R50x1', pretrained_backbone,norm_layer=GroupNorm32, trainable_layers=trainable_backbone_layers)
    model = FastWILDS(backbone, num_classes, **kwargs)
    ...
    if pretrained:
        ...
        backbone.load_from(np.load('/home/katyag/wilds/BiT-M-R50x1.npz'))
...
class FastWILDS(GeneralizedRCNN):
    def __init__(self, backbone, num_classes=None,
                 ...
                 #rpn_pre_nms_top_n_train=3000, rpn_pre_nms_top_n_test=2000,
        ...
        #min_size_list = [480,512,544,576,608,640,672,704,736,768,800]
        #min_size = np.random.choice(min_size_list,1)
