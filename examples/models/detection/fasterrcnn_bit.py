...
def resnetGN_fpn_backbone(backbone_name,weights,norm_layer= nn.BatchNorm2d,
    ...
    backbone = bit_models.KNOWN_MODELS[backbone_name](num_channels=3)
...
def _resnet_fpn_extractor(
    ...
    #return_layers = {f"body.block{k}": str(v) for v, k in enumerate(returned_layers)}
...
def fasterrcnn_resnet50_fpn(pretrained=False, progress=True,
                            num_classes=91, pretrained_backbone=True, trainable_backbone_layers=3,
                             backbone_name ='BiT-M-R50x1',backbone_path =None,
                            **kwargs):
    ...
    backbone = resnetGN_fpn_backbone(backbone_name, pretrained_backbone,norm_layer=GroupNorm32, trainable_layers=trainable_backbone_layers)
    ...
    if backbone_path is not None:
        ...
        backbone.load_from(np.load(backbone_path))
