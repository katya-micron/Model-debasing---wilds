...
def initialize_transform(
    ...
    elif additional_transform_name == "weak_v2":
        transform = add_weak_transform_v2(
...
def add_weak_transform_v2(config, dataset, base_transform_steps, should_normalize, normalization):
    # the same as the above add_weak_transform() but replace the RandomCrop with RandomResizedCrop
    target_resolution = _get_target_resolution(config, dataset)
    weak_transform_steps = copy.deepcopy(base_transform_steps)
    weak_transform_steps.extend(
        [
